...
char *C_HL_keywords[] = {
    ...
    "else", "struct", "typedef", "static", "enum", "class", "case",
...
int editorReadKey() {
    int nread;
    char keyRead;
    while ((nread = read(STDIN_FILENO, &keyRead, 1)) != 1) {
...
// Get WS from IOCTL and fill the values
...
void abAppend(struct abuf *ab, const char *s, int len) {
    // give memory block from ab.b to ab.len+len
...
void editorDrawRows(struct abuf *ab) {
    int y; 
    // Descend through each line and print content
    for (y = 0; y < E.screenrows; y++) {
        // print this when finished with all data rows
